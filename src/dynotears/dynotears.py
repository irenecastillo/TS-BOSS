...
#
# This file has been modified from its original version in the CausalNex repository
# (https://github.com/mckinsey/causalnex). The only change is that the two
# `from causalnex.structure ...` import lines have been replaced with relative
# imports pointing to local copies of StructureModel and DynamicDataTransformer,
# so that the file can be used without installing the CausalNex package
# (which is not available for Python 3.12). All function names, signatures,
# and return types are unchanged.
# Modifications made in 2026 by Irene Castillo.
"""
